...
#define WM_REMIX_UPDATE_INTERACTION (WM_USER+0x7E1+3)
...
void GameOverlay::setUiActive(bool active) {
  const bool previous = m_uiActive.exchange(active);
  if (previous == active || !m_hwnd) {
    return;
  }

  PostMessageW(m_hwnd, WM_REMIX_UPDATE_INTERACTION, 0, 0);
}
...
void GameOverlay::update(HWND gameHwnd) {
  if (m_gameHwnd == 0) {
    m_gameHwnd = gameHwnd;
    Logger::info(str::format("GameOverlay: attaching to game window ", gameHwnd));
    // Spawn UI thread
    m_thread = std::thread([this] { windowThreadMain(); });
  }
}

void GameOverlay::registerRawInputDevices(bool uiActive) {
  RAWINPUTDEVICE rid[2] {};
  rid[0].usUsagePage = 0x01;
  rid[0].usUsage = 0x02;
  rid[0].dwFlags = RIDEV_INPUTSINK | (uiActive ? RIDEV_NOLEGACY : 0);
  rid[0].hwndTarget = m_hwnd;

  rid[1].usUsagePage = 0x01;
  rid[1].usUsage = 0x06;
  rid[1].dwFlags = RIDEV_INPUTSINK | (uiActive ? RIDEV_NOLEGACY : 0);
  ...
  Logger::info(str::format("GameOverlay: raw input updated, uiActive=", uiActive ? 1 : 0));
}

void GameOverlay::updateInteractionState() {
  if (!m_hwnd) {
    return;
  }

  const bool uiActive = m_uiActive.load();
  ...
  SetLayeredWindowAttributes(m_hwnd, 0, uiActive ? 1 : 0, LWA_ALPHA);
  SetWindowPos(m_hwnd, uiActive ? HWND_TOPMOST : HWND_NOTOPMOST, 0, 0, 0, 0,
    SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE | SWP_NOOWNERZORDER | SWP_FRAMECHANGED);

  registerRawInputDevices(uiActive);
}
...
LRESULT GameOverlay::overlayWndProc(UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_REMIX_HIDE_OVERLAY:
    hide();
    return 0;
  case WM_REMIX_UPDATE_INTERACTION:
...
void GameOverlay::windowThreadMain() {
  ...
  Logger::info(str::format("GameOverlay: created overlay window ", m_hwnd, " for game window ", m_gameHwnd));
